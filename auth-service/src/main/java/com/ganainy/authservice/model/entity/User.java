 ...
 *   INSERT INTO users (email, password, first_name) VALUES ('john@example.com', 'hash123', 'John');
 * 
...
public class User {
     ...
     *   - SEQUENCE: Uses a database sequence (more control, PostgreSQL preferred)
     *   - AUTO: Let Hibernate choose the strategy
     *   - TABLE: Uses a special table to track IDs (rarely used)
     * 
     * We use IDENTITY for simplicity. The database will auto-generate IDs:
     * 1, 2, 3, 4, 5...
     */
    @Id
     ...
     *   - ORDINAL: Stores the position (0, 1, 2...) - DANGEROUS if enum order changes!
     *   - STRING: Stores the name ("ADMIN", "DOCTOR") - SAFE and readable
     * 
     * Always use EnumType.STRING to avoid bugs when you add/reorder enum values.
     */
